...
"""
    ...
    def _get_volttron_system_prompt(self):
        ...
        return (
            ...
            "- Example: User says 'weather monitor' → That's the agent name, not a request for weather info\n"
            "- Example: User says 'cat facts' → That's the agent name, not a request for cat facts\n"
            "- **USE CONTEXT FROM PREVIOUS STEPS**: When at Step 2+, check agent_creator_state to see what user already answered\n"
            "  - Step 2 (VIP Identity): If agent name is 'catfacts', suggest 'service.catfacts' or 'api.catfacts' based on that name\n"
            ...
            "- Stay in wizard mode until user says 'cancel', 'exit', 'quit' or wizard completes\n"
            "\n"
            "TOOLS AVAILABLE:\n"
